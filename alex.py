...
def get_artist_stats(x):
	x = x[0][0]
	json_data = correct_json(x)
	if len(json_data) <= 0:
		return
	try:
		json_data = json_data.encode('ascii', 'ignore')
		json_data = json.loads(json_data)
		artist_name = json_data.keys()[0]
		artist_pop = json_data[artist_name]['popularity']
		artist_sum = json_data[artist_name]['summary']
		artist_content = json_data[artist_name]['content']
		artist_followers = json_data[artist_name]['followers']
		artist_genres = json_data[artist_name]['genres']
		artist_album_num = 0
		artist_song_num = 0 
		artist_duration = 0
		artist_albums = []
		...
		track_popularity = []
		track_duration = []
		for i in json_data[artist_name]['albums']:
		...
		artist_ref_count = len(json_data[artist_name]['references'])
...
def main():
	...
	for i in range(1, len(sys.argv)):
		...
		words = textRDD.map(lambda x: x.split("--------------------------------------")).map(lambda x: (x,1))

		artistRDD = words.map(lambda x: get_artist_stats(x)).distinct().filter(lambda x: x is not None).filter(lambda x: x!= "").filter(lambda row: row.artist_name != "" and row.artist_name != None)
